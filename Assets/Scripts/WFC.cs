using Assets.Scripts.Solver;
using System;
using System.Collections;
using System.Collections.Generic;
using System.Linq;
using UnityEditor;
using UnityEngine;
using UnityEngine.SceneManagement;

public class WFC : MonoBehaviour
{
	// Start is called before the first frame update

	public Vector3Int Size = new Vector3Int(3, 4, 3);
	public int SlotSize = 1;

	public GameObject debug_spawner;
	public GameObject debug_spawner_2;

	public bool DrawDebug = true;
	public long DebugSpeed = 200;

	public int HorizontalConnector = 0;
	public int UpConnector = 0;
	public int DownConnector = 0;


	public WFCPrototype[] prototypes;

	private Slot[,,] grid;

	public GameObject SceneLoader { get; internal set; }

	public void ClearChildren()
	{
		for (int i = transform.childCount - 1; i >= 0; --i)
		{
			DestroyImmediate(transform.GetChild(i).gameObject);
		}
	}
	public void StartGeneration()
	{
		Debug.Assert(prototypes != null, "Prototypes was not generated");
		grid = new Slot[Size.x, Size.y, Size.z];

		for (int x = 0; x < Size.x; x++)
		{
			for (int y = 0; y < Size.y; y++)
			{
				for (int z = 0; z < Size.z; z++)
				{
					grid[x, y, z] = new Slot(prototypes.Select(p => p.Id));
				}
			}
		}

		// Back and forward sides
		/*for (int x = 0; x < Size.x; x++)
		{
			for (int y = 0; y < Size.y; y++)
			{
				grid[x, y, 0].SetLockedDirection(SlotDirection.BACK, new[] { HorizontalConnector }, prototypes);
				grid[x, y, Size.z - 1].SetLockedDirection(SlotDirection.FORWARD, new[] { HorizontalConnector }, prototypes);
			}
		}
		*/
		/*for (int y = 0; y < Size.y; y++)
		{
			for (int z = 0; z < Size.z; z++)
			{
				grid[0, y, z].SetLockedDirection(SlotDirection.BACK, new[] { HorizontalConnector }, prototypes);
				grid[Size.x - 1, y, z].SetLockedDirection(SlotDirection.FORWARD, new[] { HorizontalConnector }, prototypes);
			}
		}*/
		for (int z = 0; z < Size.z; z++)
		{
			for (int x = 0; x < Size.x; x++)
			{
				grid[x, 0, z].SetLockedDirection(SlotDirection.DOWN, new[] { DownConnector }, prototypes);
				grid[x, Size.y - 1, z].SetLockedDirection(SlotDirection.UP, new[] { UpConnector }, prototypes);
			}
		}

		Solver solver = new Solver(grid, prototypes.ToList());
		solver.Completed += (_, result) =>
		{
			Debug.Log($"Completed with {result.Backtracks} backtracks");
			for (int x = 0; x < Size.x; x++)
			{
				for (int y = 0; y < Size.y; y++)
				{
					for (int z = 0; z < Size.z; z++)
					{
						var v = result.Grid[x, y, z].Value;
						var pos = new Vector3(x, y, z);
						var go = Instantiate(prototypes[v].GetGameObjectPrototype(), pos, prototypes[v].GetRotation(), this.transform);

					}
				}
			}
		};
		solver.Failed += (_, backtracks) =>
		{
			Debug.LogError($"No solution could be found with {backtracks} backtracks.");
		};

		solver.Solve();

		grid = null;
	}

	private void OnDrawGizmos()
	{
		Gizmos.color = Color.green;
		Vector3 position = transform.position + (Vector3)(Size * SlotSize) * 0.5f;
		Gizmos.DrawWireCube(position, Size * SlotSize);

	}
}
