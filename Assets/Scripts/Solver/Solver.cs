	...
	public class Solver
	{
		...
		public void Solve()
		{
			...
			while (true)
			{
				...
				Profiler.BeginSample("FindSmallestDomain");
		...
		private Slot FindSmallestDomain()
		{
			...
			{
				Debug.Assert(x.DomainSize > 0, "Domain size is 0 when searching");
		...
		private PropagationState Propagate(Slot start)
		{
			...
			while (stack.Count > 0)
			{
				...
				{
					foreach (var neighbour in node.Neighbours[direction])
					{
